...
class Article extends CI_Controller {
    ...
    public function category() {
        if($this->uri->segment(4)){
            $cat_row = $this->Csz_model->getValue('*', 'article_db', "is_category = '1' AND url_rewrite = '".$this->uri->segment(4)."'", '', 1);
            $row = $this->Csz_model->load_config();
            $this->template->set('title', 'Article | ' . $row->site_name);
            $this->template->set('meta_tags', $this->Csz_model->coreMetatags('CSZ CMS | Article',$row->keywords));
            $this->template->set('cur_page', $this->page_url);
    ...
    public function view() {
        if($this->uri->segment(4)){
            $art_row = $this->Csz_model->getValue('*', 'article_db', "article_db_id", $this->uri->segment(4), 1);
            $row = $this->Csz_model->load_config();
            ...
            $cat_row = $this->Csz_model->getValue('category_name', 'article_db', "is_category = '1' AND article_db_id = '".$art_row->cat_id."'", '', 1);
            $this->template->setSub('category_name', $cat_row->category_name);
