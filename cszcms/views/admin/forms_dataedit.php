...
<div class="row">
    <div class="col-lg-12 col-md-12">
        ...
        <?php echo form_open_multipart($this->Csz_model->base_link(). '/admin/forms/dataUpdate/'.$this->uri->segment(4).'/'.$this->uri->segment(5)); ?>
        <?php $html = '';
        if ($field_data !== FALSE) {
            foreach ($field_data as $field) {
                $field_name_post = $field['field_name'];
                ...
                if ($field['field_type'] == 'number' || $field['field_type'] == 'email' || $field['field_type'] == 'password' || $field['field_type'] == 'text' || $field['field_type'] == 'datepicker' || $field['field_type'] == 'timepicker' || $field['field_type'] == 'textarea') {
                    $field['field_value'] = $post->$field_name_post;
                }
                if ($field['field_type'] == 'number' || $field['field_type'] == 'email' || $field['field_type'] == 'file' || $field['field_type'] == 'password' || $field['field_type'] == 'text') {
                    ...
                    if($field['field_type'] == 'file' && $post->$field_name_post){
                        $html.= '<div class="row"><div class="col-md-6" style="word-wrap: break-word;"><a href="' . base_url() . "photo/forms/" . $this->Csz_model->cleanEmailFormat($form_name) . '/' . $this->Csz_model->cleanEmailFormat($field['field_name']) . '/' . $post->$field_name_post . '" target="_blank">' . base_url() . "photo/forms/" . $this->Csz_model->cleanEmailFormat($form_name) . '/' . $this->Csz_model->cleanEmailFormat($field['field_name']) . '/' . $post->$field_name_post . '</a></div>';
                        $html.= '<div class="col-md-6"><label><input type="checkbox" name="' . $field['field_name'] . '_delfile" value="photo/forms/' . $this->Csz_model->cleanEmailFormat($form_name) . '/' . $this->Csz_model->cleanEmailFormat($field['field_name']) . '/' . $post->$field_name_post . '"> <span class="remark">'.$this->lang->line('btn_delete').'</span></label></div></div><br>';
                        $html.= '<input type="hidden" name="' . $field['field_name'] . '" value="">';
                    }else{
                    ...
                    if($post->$field_name_post){
                        $checked = ' checked="checked"';
                    }else{
                        $checked = '';
                    }
                    $html .= '<div' . (($field['field_div_class']) ? ' class="' . $field['field_div_class'] . '"' : '') . '>';
                    $html .= '<label for="' . $field['field_id'] . '"><input type="' . $field['field_type'] . '" name="' . $field['field_name'] . '" value="' . $field['field_value'] . '" id="' . $field['field_id'] . '" class="' . $field['field_class'] . '" placeholder="' . $field['field_placeholder'] . '"' . $f_req . $checked . '/>   ' . $field['field_label'] . $star_req . '</label>';
                    $html .= '</div>';
                    ...
                    if ($field['sel_option_val']) {
                        ...
                        foreach ($opt_arr as $opt) {
                            ...
                            if($post->$field_name_post && $post->$field_name_post == trim($val)){
                                $selected = ' selected="selected"';
                            }else{
                                $selected = '';
                            }
                            $opt_html .= '<option value="' . trim($val) . '"'.$selected.'>' . trim($show) . '</option>';
                        }
                    }
                ...
                if($dont_repeat_field){
                    $html .= '<input type="hidden" name="'.$dont_repeat_field.'_old" value="'.$post->$dont_repeat_field.'">';
                }
            }
        }
        echo $html;
